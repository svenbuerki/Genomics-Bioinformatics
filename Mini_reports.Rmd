...
4. Compare the resulting sequences against reference databases to find the matching species. Here we will be using DNA sequences from [GenBank](https://www.ncbi.nlm.nih.gov/genbank/) as our reference. See Chapter 3 for more details on GenBank.
...
5. Infer phylogenetic analysis based on Maximum Likelihood criterion as implemented in RAxML [@Stamatakis2014]. This phylogenetic framework will be used to test and refine species working hypotheses (as inferred in learning outcome #2).
...
for(i in 1:length(GBresults$ids)){
  ...
  #Print i = sample if you need to restart the loop
