...
Like porcini [@dentinger2014s], vanilla is one of our most beloved spice, but we still know very little about its biology, taxonomy and evolution. As part of a study aiming at inferring domestication processes in the genus *Vanilla* and species adaptation to drought, fieldwork was conducted in Mexico by Paige Ellestad (an EEB Ph.D. student at BSU) and her in-country partners.
...
## Data and project structure for mini-report

For the purpose of this mini-report, **we will be focusing on four samples/individuals of vanilla (see Figure \@ref(fig:van)) and their associated ITS sequence data**.
...
Finally, `Vanilla_samples_records.xlsx` contains meta-data information about the samples. We will also update this file with information on species hypotheses.
...
Students will thrive at answering the following questions:

> Q1: To what species do the individuals presented in Figure \@ref(fig:van) belong to?
>
> Q2: How are those individuals related to each other?

To answer these questions, we will be analyzing the `.ab1` ITS data and use DNA sequences available on GenBank as our reference library.

## Schedule

Analyses for this report are subdivided into two parts. Please find below a preliminary agenda for completion of each part:

- **Part 1:** Process and clean ITS DNA sequence electropherograms using [`FinchTV`](https://digitalworldbiology.com/FinchTV). Validate DNA sequence and provide species working hypothesis by conducting a BLAST similarity DNA sequences analysis coupled with a distance tree inference. This will be based on DNA sequences deposited on GenBank.
- **Part 2:** GenBank query and preparing data.
- **Part 2:** Perform DNA multiple sequence alignments using `MAFFT` and infer ML analysis using `RAxML`. Finally, visualize and produce phylogenetic tree figures showing positions and relationships of individuals using `R`.

## Part 1: Processing DNA electropherograms and inferring working species hypothesis
...
4. Merge individual `FASTA` objects using `rbind()`.
5. Write output `FASTA` object into file using `write.table()`. 
...
for(i in 1:length(Files)){
  ...
  tmp <- paste(tmp[1], paste(tmp[2:length(tmp)], collapse = ''), sep='\n')
  #Merge objects
...
```{r mergefasta, eval=F}
###~~~
#Start by merging datasets (S1: GenBank and S2: your newly produced seq.)
###~~~
...
4. Conduct the msa analysis using the `MAFFT` algorithm (available on a web platform).
5. Check and manually edit msa. This will be done by using either `MEGA` or `AliView` software.
6. Infer ML phylogenetic tree based on cleaned msa files. This will be done using the `RAxML` algorithm (available on a web platform).
7. Visualize phylogenetic tree and interpret results. 
