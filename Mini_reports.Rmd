...
To perform the ML phylogenetic analysis, please go on the webpage and follow the procedure using default parameters with the exception of the `Bootstrapping settings`. These have to be set as shown in Figure \@ref(fig:RAxML).

```{r RAxML, echo=FALSE, fig.align="center", fig.cap="Screenshot showing Bootstrapping settings for RAxML analysis.", fig.show="asis", out.width = '100%'}
knitr::include_graphics("Images/RAxML_online.png")
```

As input file, please use the output of the `MEGA` analysis (`*_GenBank_seq_msa_output.fasta`), which is stored in `04_Data_analyses/FASTA/`. Finally, don't forget to provide your email before submitting the analysis. The analysis should take ca. 30 minutes to run. You will receive an email to download results. 

Please save the output files those we will be analyzed in PART3. If you cannot wait, download `FigTree` and open the `.tre` file to look at your phylogeny!
