...
For this assignment, students will investigate the following questions:

> Q1: To what species of vanilla do the individuals presented in Figure \@ref(fig:van) belong to?
>
> Q2: How are those individuals related to each other?

To answer these questions, **we will be focusing on four samples/individuals of vanilla (see Figure \@ref(fig:van)) and their associated ITS sequence data as well as ITS sequences deposited on GenBank**.
...
plot(read.tree(text=tr3), show.node.label = TRUE)
```
...
1. Load ITS tree using *read.tree*.
...
4. Root phylogenetic tree with outgroup taxon. Analyses conducted by Paige showed that "1789804116_Vanilla_inodora" is suitable outgroup taxon.
5. Ladderize and plot phylogenetic tree (Figure \@ref(fig:Vantreeall)). We will also color the tips that are matching your vanilla samples in red.
6. Extract subtree containing our vanilla samples and plot tree (this time using the radial plotting mode; see Figure \@ref(fig:Vantreesub)). To visualize your vanilla samples we will plot a red circle next to their tips and rename them based on Figure \@ref(fig:van). 
...
ITS <- read.tree(file="Data/04_Data_analyses/Phylogenetic_analyses/result_raxml_45818/result.raxml.support")
...
ITS <- root(ITS, outgroup = "1789804116_V._inodora", resolve.root=T)
...
#Plot
plot.phylo(ITS, cex=.3, tip.color = tipcol, show.node.label = TRUE, align.tip.label = T, no.margin = TRUE)
```
...
MRCAsamples <- getMRCA(ITS, tip=ITS$tip.label[grep("-ITSp4", ITS$tip.label)])

#Extract subtree
VanITS <- extract.clade(ITS, node=MRCAsamples)
...
plot.phylo(VanITS, cex = .4, use.edge.length = F, tip.color = tipcolVan, type = "radial", label.offset = 0.04, no.margin = TRUE)
#Add bootstrap supports
nodelabels(text = VanITS$node.label, adj = c(0.5,0.5), frame = "none", cex = 0.5)
#Add circles next to your vanilla samples
tiplabels(tip = grep("-ITSp4", VanITS$tip.label), pch = 16, col = "red", offset = 0.02)
```
