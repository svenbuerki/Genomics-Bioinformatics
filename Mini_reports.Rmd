...
- [**Mini-Report 3**](#mini3): Apply a coupled DNA barcoding and phylogenetic approach to identify species and infer their evolutionary history (50 points). This report provides students with an introduction to methods applied in the laboratory sessions and expertise to select a comparative article (Chapter 8).
...
Time will be allocated in class on the 21st of January 2022 to allow students to work on their assignments. Reports are due on **the 4th of February 2022** and should be uploaded on the shared [Google Drive](https://drive.google.com/drive/folders/1BFZFJeR8tW8N7QLfngqIVbjqEkDuIyWj?usp=sharing) in the `Mini_Reports` folder.
...
Upload your reports on the shared Google drive by the 4th of February before 5 pm. Name your report documents following this pattern: `Sequencing platform_Surname`.

## Resources

The instructor has provided `pdf` documents devoted to each sequencing platform below to support students in starting their assignments. 
...
Time will be allocated in class on the 4th of February 2022 to allow students to work on their assignments. Reports are due on **the 18th of February 2022** and should be uploaded on the shared [Google Drive](https://drive.google.com/drive/folders/1BFZFJeR8tW8N7QLfngqIVbjqEkDuIyWj?usp=sharing) in the `Mini_Report_2` folder.
...
Maximum two pages, single spaced, using either Arial 11 pt or Times New Roman 12 pt and with all margins set to 1 inch. The two pages do not include a title page and references. The title page should include: name of the molecular database as title and name of student contributing the report. Please keep in mind that a figure can convey a lot more information than a long text! Your reports should follow the structure presented in section \@ref(structure2). 

Upload your reports on the shared Google drive by the **the 18th of February 2022** (5 PM MT). Name your report documents following this pattern: `Molecular database_Surname`.
...
# Mini-report 3 -- Apply a coupled DNA barcoding and phylogenetic approach to identify species and infer their evolutionary history {#mini3}
...
- inferring evolutionary frameworks allowing developing working hypotheses on species relationships and their spatio-temporal histories (e.g. knowing how did species cope with past climatic conditions will provide insights into their adaptive capacity to forecasted conditions). 
...
**DNA barcoding** is a method of species identification using a short section of DNA from a specific gene or genes. The premise of DNA barcoding is that, by comparison with a reference library of such DNA sections, an individual DNA sequence can be used to uniquely identify an organism to species, in the same way that a supermarket scanner uses the familiar black stripes of the UPC barcode to identify an item in its stock against its reference database [@CBOL]. These DNA barcodes are sometimes used in an effort to identify unknown species, parts of an organism, or simply to catalog as many taxa as possible, or to compare with traditional taxonomy in an effort to determine species boundaries. For more details on DNA barcodes applied to plants, please see section [below](#vanbarcode) dedicated to vanilla.

DNA barcoding has a wide range of applications from biodiversity surveys [e.g. @telfer2015biodiversity], illegal trade of wildlife [e.g. @gonccalves2015dna] to food security [e.g. @quinto2016dna]. The DNA barcoding approach has been recently reviewer by @deSalle2019. 

#### Procedure

A DNA barcoding approach is subdivided into 4 steps:
...
4. Compare the resulting sequences against reference databases to find the matching species. Here we will be using DNA sequences from [GenBank](https://www.ncbi.nlm.nih.gov/genbank/) as our reference.
...
A phylogenetic analysis is subdivided into 4 steps:
...
4. Infer phylogenetic tree analysis (usually based either on Maximum Likelihood and/or Bayesian criteria).
...
2. Validate DNA sequence and provide species working hypothesis by conducting a BLAST similarity DNA sequences analysis coupled with a distance tree inference. This will be based on DNA sequences deposited on GenBank.
3. Query GenBank and download DNA sequences for phylogenetic analyses using R packages.
4. Produce DNA multiple sequence alignments (containing your target DNA sequences and DNA sequences deposited on GenBank) using freely available algorithms.
5. Infer phylogenetic analysis based on Maximum Likelihood (ML) criterion as implemented in RAxML [@Stamatakis2014]. This phylogenetic framework will be used to test and refine species working hypotheses (as inferred in learning outcome #2).
6. Visualize and interpret phylogenetic analysis.
7. Learn the terminology associated to DNA barcoding and phylogenetic analyses. Several definitions associated to the most used terms is available in the [Lexicon](Lexicon.html).

To support learning these skills, we will be using a DNA dataset focusing on the [vanilla](#vanilla) spice (from the Orchidaceae family). 

## Vanilla as model for this report {#vanilla}
...
### DNA barcodes generated for vanilla project {#vanbarcode}
...
## Scientific questions associated to the mini-report {#sciq}
...
## Scientific process {#sciprocess}

The scientific process associated to this assignment can  be summarized as follows (see Figure \@ref(fig:sciproc)):

```{r sciproc, echo=FALSE, fig.align="center", fig.cap="The scientific process.", fig.show="asis", out.width = '80%'}
knitr::include_graphics("Images/Scientific_process.png")
```

1. To what species of vanilla do the individuals presented in Figure \@ref(fig:van) belong to?
    - **Hypothesis:** Individuals belong to the same species (*V. planifolia*) and observed phenotypic variation is due to phenotypic plasticity (exhibiting responses to contrasting climates). 
2. To test the hypothesis we are applying the **phylogenetic species concept** [@wheeler1999phylogenetic] (mainly criterion of monophyly) based on DNA sequence data analyzed using a phylogenetic approach. 
3. ITS barcode data are sequenced for individuals in Figure \@ref(fig:van) and compared to data from GenBank (= reference data).
4. Analyses are done in 4 steps:
    - PCR and Sanger sequencing of ITS barcodes for target individuals.
    - Assembly of reference ITS data available on GenBank using R.
    - BLAST and RAxML Maximum likelihood phylogenetic analysis (including bootstrap analysis).
    - Produce figures using R.
5. Intersect evidence from analyses (especially phylogenetic clustering combined with node supports and taxonomy of ITS sequences from GenBank) to infer to what species do target individuals belong to. Use criterion of monophyly to help circumscribe species.
6. See section \@ref(report) for more details.

## Writing your report {#report}

The instructor provides below information to complete the Mini-Report 3. All the material, data, code and references required to complete this report are provided here and were studies in class. In this context, students will have to focus on formatting their reports following guideline presented here as well as making sure that their `R` code is working and ready to be shared. 
...
### Structure of the Mini-Report 3
...
Individual reports should be submitted in either Google Docs, Word or Rmarkdown formats (and if you decide to submit an additional file containing your R script, submit this file as an `.R` format). These files should be uploaded on the shared Google Drive in the `Mini_Report_3` folder. Name your report documents following this pattern: `Mini_Report_3_Surname`.

**Upload your reports on the shared Google drive by the 10th of March before 5 PM.** 

## Schedule

Analyses for this report are subdivided into three parts:
...
**Individual reports are due on Wed 10th of March at 5 PM** (on Google Drive). Please see section \@ref(report) for more details.

## Data and project structure

The data are deposited on the shared [Google Drive](https://drive.google.com/drive/folders/1BFZFJeR8tW8N7QLfngqIVbjqEkDuIyWj?usp=sharing) in a folder entitled `DNA_barcoding`. All students enrolled in this class have been granted access to this folder. The folder is subdivided into four sub-folders (see Figure \@ref(fig:datastr)) and contains a master spreadsheet `Vanilla_samples_records.xlsx` at its root: 
...
We will be discussing this topic further during our Zoom meeting.
...
2. Launch `FinchTV`.
3. Open `.ab1` file (one at a time) using the `Open` tab or the folder icon. 
...
15. Open `Vanilla_samples_records.xlsx` and update `Species_BLAST` column with a species name (your first working hypothesis). Don't forget to save the file.
...
1. Expand sampling for phylogenetic analyses by downloading ITS DNA sequences of *Vanilla* species deposited on GenBank. This will be done by using the *rentrez* `R` package. A table with species taxonomy associated to GenBank accessions will also be built. We will also clean the data and save outputs in `FASTA` and `csv` formats.
2. Format and merge individual `.seq` files from Part 1 into a `FASTA` object/file. This will be done by creating a list of all `.seq` files, opening them individually and merging them into one object using a `for` loop.
...
Before delving into the code, start by creating an `.R` file in `RStudio` and save it in the same working directory as where you want to save the output files (e.g. `DNA_barcoding/04_Data_analyses`).
...
###~~~
RawGBDat <- read.csv("~/Documents/Class_Genomics&Bioinfo_Spring/DNA_barcoding/04_Data_analyses/CSV/Instructor_files/Vanilla_internal transcribed spacer_2021_02_11_Raw_GenBank.csv", quote = "\"", sep=' ')
...
#Load Raw GB query csv file
###~~~
RawGBDat2 <- read.csv("~/Documents/Class_Genomics&Bioinfo_Spring/DNA_barcoding/04_Data_analyses/CSV/Instructor_files/Vanilla_internal transcribed spacer_2021_02_11_GenBank.csv", quote = "\"", sep=' ')
...
Please notice that steps 2 to 4 will take place within a `for` loop.
...
Branch lengths can be incorporated into a tree by putting a real number, with or without decimal point, after a node and preceded by a colon (`:`). This represents the length of the branch immediately below that node. Thus, the above tree might have lengths represented as in Figure \@ref(fig:newickbrlens):
...
Now, we can finally add node supports by simply adding values after each closing parenthesis. This is done as follows and displayed in Figure \@ref(fig:newickbrlensnode):
...
4. Root phylogenetic tree with outgroup taxon (using the *ape::root()* function). Analyses conducted by Paige showed that *Vanilla inodora* (represented by the sample `1789804116_Vanilla_inodora`) is a suitable outgroup taxon.
5. Ladderize and plot phylogenetic tree (using *ape::ladderize()* and *ape::plot.phylo()* functions; see Figure \@ref(fig:Vantreeall)). We will also color in red the tips associated to your vanilla samples.
6. Extract subtree containing our vanilla samples (using *ape::getMRCA()* and *ape::extract.clade()* functions) and plot tree (this time using the radial plotting mode; see Figure \@ref(fig:Vantreesub)). To visualize your vanilla samples we will plot a red circle next to their tips (using the *ape::tiplabels()* function) and rename them based on Figure \@ref(fig:van). 
