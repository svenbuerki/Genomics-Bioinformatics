...
###~~~
RawGBDat <- read.csv("~/Documents/Class_Genomics&Bioinfo_Spring/DNA_barcoding/04_Data_analyses/CSV/Instructor_files/Vanilla_internal transcribed spacer_2021_02_09_Raw_GenBank.csv", quote = "\"", sep=' ')
...
#Load Raw GB query csv file
###~~~
RawGBDat2 <- read.csv("~/Documents/Class_Genomics&Bioinfo_Spring/DNA_barcoding/04_Data_analyses/CSV/Instructor_files/Vanilla_internal transcribed spacer_2021_02_09_GenBank.csv", quote = "\"", sep=' ')
...
1. Conduct the msa analysis using the `MAFFT` algorithm (available on a web platform). This will be done using `Vanilla_internal transcribed spacer_2021_02_09_GenBank_seq_msa_input.fasta` stored in `04_Data_analyses/FASTA/`.
2. Check and manually edit msa. This will be done by using either `MEGA` or `AliView` software.
3. Infer ML phylogenetic tree based on cleaned msa files. This will be done using the `RAxML` algorithm (available on a web platform).
