...
**Upload your reports on the shared Google drive by the 18th of March before 5 PM MT.** 
...
**Tip**: To execute a line of source code where your cursor currently resides you press the `Ctrl+Enter` key (or use the Run toolbar button; see Figure \@ref(fig:IDE)).

### Analytical workflow 

To infer the ML phylogenetic analysis, the following workflow will be executed:

1. Expand sampling for phylogenetic analyses by downloading ITS DNA sequences of *Vanilla* species deposited on GenBank. This will be done by using the *rentrez* `R` package [@rentrez]. A table with species taxonomy associated to GenBank accessions will also be built. We will also clean the data and save outputs in `FASTA` and `csv` formats.
...
SEQfileName <- "Vanilla_internal transcribed spacer_2022_02_18_Raw_GenBank.csv"
