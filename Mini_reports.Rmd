...
To perform the Maximum Likelihood (ML) phylogenetic analysis, the following workflow will be implemented:

1. Expand taxon sampling for phylogenetic analyses by downloading ITS DNA sequences of *Vanilla* species deposited in GenBank using the following approach:  
    - Use functions from the *rentrez* `R` package [@rentrez] to download DNA sequences and associated metadata.  
    - Build a table linking species taxonomy to GenBank accession numbers.  
    - Clean the dataset and save the outputs in `FASTA` and `csv` formats.  

2. Format and merge individual `.seq` files generated in Part 1 into a single `FASTA` file using the following approach:  
...
3. Combine the `FASTA` files generated in steps 1 and 2 and perform a multiple sequence alignment (MSA).

Detailed protocols associated with each step of this workflow are provided in the sections below.
...
You are expected to **carefully review, run, and understand the code**, and then **adapt it to retrieve the ITS sequences required for your analysis**. The workflow is organized into the following steps:

1. [Get familiar with the required R packages and workflow structure](#introentrez)  
...
#### Get Familiar with Required R Packages and Workflow Structure {#introentrez}

##### Overview of the Approach

The following pseudo-code is applied to download (here referred to as "fetch") data from Entrez using the R *rentrez* package [@rentrez]:
...
3. Save the `.R` script at the root of your project (`DNA_barcoding/`) with the following name `01_PART2_YOUR_NAME.R`.

All the R code provided below will be copied into your newly saved `R` script and executed in class together. The full R script can be downloaded on [Google Drive](https://drive.google.com/drive/folders/1iBMwP67FoSTMDX-EpXOlf8utP9k8tu8k?usp=drive_link).
