...
The goal of this course is to provide students with the theoretical and applied knowledge in genomics and bioinformatics to sequence, assemble and annotate genomes, especially for non-model organisms (see Figure \@ref(fig:flowchart)). To provide an authentic research experience, we will be using examples from the instructor's research on sagebrush (*Artemisia tridentata*; Asteraceae) genomics [e.g. @Melton2021].
...
A tentative timetable for the Spring semester 2022 is presented in Table \@ref(tab:timetab).
...
#From <- "2021/01/11"
#To <- "2021/04/30"
dates <- data.frame(seq.Date(from=as.Date("2022/01/10"), to=as.Date("2022/04/29"), by="day"))
...
#From <- "2021/04/12"
#To <- "2021/04/16"
holiday <- seq.Date(from=as.Date("2022/03/21"), to=as.Date("2022/03/25"), by="day")
...
knitr::kable(timetable,  caption = "Tentative timetable for the Genomics and Bioinformatics course during the Spring semester 2022.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

#PDF
#landscape(knitr::kable(timetable, caption = "Tentative timetable for the Genomics #and Bioinformatics course during the Spring semester 2022.", booktabs = T, #"latex") %>% kable_styling(latex_options = c("striped","scale_down"))
#          )
```
