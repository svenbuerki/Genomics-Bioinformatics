---
title: "Tutorials"
subtitle: "Bioinformatics toolkit for genomics"
date: "`r Sys.Date()`"
output:
...
This webpage contains several tutorials providing introductions to different facets of the Linux operating system here focusing on the [Ubuntu](https://en.wikipedia.org/wiki/Ubuntu) distribution. Information presented here are especially relevant for [Chapter 4](https://svenbuerki.github.io/Genomics-Bioinformatics/Chapters.html#6_Chapter_4) and the [Lab assignment](https://svenbuerki.github.io/Genomics-Bioinformatics/Mining_draft_genome.html). 

# Learning outcomes

1. Learn procedure to [remotely access](#remote) linux computers using the `ssh` protocol.
2. Learn procedure to [safely run computer jobs](#runjob) using the `tmux` protocol.
3. Learn [Linux commands](#lcmd) to conduct genomic analyses. 

# Your computer accounts

Details on your Linux computer accounts and their IP addresses are summarized in this [Google document](https://docs.google.com/document/d/1t0DdQBes42Gzo8XWgc9JL2xEGjv1kbfoVE7Vznx6C9I/edit?usp=sharing). Your assigned user ID are provided in this [spreadsheet](https://docs.google.com/spreadsheets/d/1cRM1yasUfDa98ecyz_-RxUOw8cU4wkQx4O5Wxw3y4yU/edit?usp=sharing).

# Remotely access Linux computers {#remote}

To remotely access your assigned Linux computer accounts using `ssh` protocol, you need to know:

1. Your user ID and password (see [Google document](https://docs.google.com/document/d/1t0DdQBes42Gzo8XWgc9JL2xEGjv1kbfoVE7Vznx6C9I/edit?usp=sharing)).
2. The IP address of the computer you want to connect to. If you want to remotely access the lab computers from a Windows machine, please read the protocol described [below](#Putty) before going any further into this document.

**Warning:** If you are accessing the Linux computers outside of BSU campus, you will need to use the VPN protocol.

## What is an IP address?

An **I**nternet **P**rotocol address is a numerical label such as 192.0.2.1 that is connected to a computer network that uses the Internet Protocol for communication. An IP address serves two main functions: 
...
Once you have gathered all required information (i.e., IP address and username) and installed [Putty](#Putty) if you have a Windows operating system, please open a `Terminal` or command prompt window and type the following command to remotely access your account using the `ssh` protocol (see Figure \@ref(fig:ssh)):
...
2. Remotely connect to your account using the `ssh` protocol
3. Exit your remote terminal session using `exit`
...
One you have executed the commands, you will be able to **rename your session** as wished and when you are finished type `Return` or `Enter` to complete the task (Figure \@ref(fig:tmuxrename)).
...
Please do the following short exercises on your assigned computer account (see \autoref{tab:tab_grp}) to get accustomed with the material presented in this document.
...
1. Remotely connect to your account using `ssh`
2. List the content of the home directory (`~`) using `ls`
3. Navigate to `Documents/` using `cd`
4. Create a new folder entitled `My_jobs` using `mkdir` and use `ls` to check that it was created.
5. Navigate in `My_jobs/`
6. Create a new text file entitled `README.txt` using `vim`. This will open a new window to edit your file
7. Edit your text file as follows:
...
8. Check that your file exists and that it has a size (= not empty) by using `ls -alh`
    - What is the size of `README.txt` in kilo bytes?
9. Delete `README.txt` using `rm`
10. Check that `README.txt` has been deleted
11. Go back to the home directory using `cd ..`
12. Remove `My_jobs/` using `rm -r`
13. Check that `My_jobs/` has been deleted
14. Exit your remote terminal session using `exit`
